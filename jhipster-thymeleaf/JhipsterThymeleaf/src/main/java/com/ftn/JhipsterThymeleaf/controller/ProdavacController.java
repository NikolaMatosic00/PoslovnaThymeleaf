...
public class ProdavacController {
    ...
    public ModelAndView get(HttpSession ses, HttpServletResponse res) throws IOException {
        ...
        List<Prodavac> list = service.findAll();
        ModelAndView response = new ModelAndView("prodavacGet");
    ...
    public ModelAndView create() throws IOException {
        ModelAndView response = new ModelAndView("prodavacCreate");

        response.addObject("req", new Prodavac());

        return response;
    }

    @PostMapping(value = "/Create")
    public ModelAndView create(@ModelAttribute Prodavac req, HttpSession ses, HttpServletResponse res) throws IOException {
        service.save(req);
        return get(ses, res);
    }
